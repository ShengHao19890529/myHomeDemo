    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            error: function (res) {
                ...
                if (!silent) that.$tableLoading.hide();
                if(res.responseText){
                    var exObj = jQuery.parseJSON(res.responseText);
                    if(exObj){
                        if(exObj.exCode=='2006'){
                            top.document.location.href = globalReqUrl == '' ? '/' : globalReqUrl;
                        }else{
                            top.pscMsg.error("系统错误[代码("+exObj.exCode + "),编号(" + exObj.exNo+"),消息("  + exObj.message +")]");
                        }
                    }
                }
            }
